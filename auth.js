...
function AuthGate() {
  ...
  useEffect(() => {
    ...
    const { data: sub } = supabaseClient.auth.onAuthStateChange((event, sess) => {
      setSession(sess);
      if (event === 'PASSWORD_RECOVERY') { setAuthMode('set-password'); setStatus('signed-out'); return; }
  ...
  if (authMode === 'set-password') return h(SetPasswordScreen, { onDone: () => setAuthMode('login') });
